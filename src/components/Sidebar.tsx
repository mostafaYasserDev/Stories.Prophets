...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const seriesGroups = useMemo(() => {
    ...
    episodes.forEach((ep, originalIndex) => {
      const eraTitle = (ep.era && ep.era.trim()) || 'قصص الأنبياء وسيرة الرسول';
    ...
    filtered.forEach((item) => {
      const eraTitle = (item.ep.era && item.ep.era.trim()) || 'قصص الأنبياء وسيرة الرسول';
      const key = normalizeArabicText(eraTitle);
      const grp = groupsMap.get(key);
      if (grp) {
      ...
      <aside className={`episodes-sidebar ${isOpenMobile ? 'drawer-open' : ''}`}>
        ...
        <div className="sidebar-mobile-title-bar">
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontWeight: 700, fontSize: '0.95rem' }}>فهرس قصص الأنبياء وسيرة الرسول</span>
          </div>
          <button
            type="button"
        ...
        <div className="sidebar-header">
          ...
          <div className="search-box">
            ...
            <input
              ...
              placeholder="ابحث في قصص الأنبياء وسيرة الرسول..."
            />
            {searchQuery && (
              <button
                type="button"
