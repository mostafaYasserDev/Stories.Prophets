...
اقرأ المقطع التالي من قصص الأنبياء وسيرة الرسول وفق القواعد الإلقائية والنطقية الدقيقة الآتية:
1. الأصل والأولوية المطلقة هي للغة العربية الفصحى السليمة، بمخارج حروف فصيحة منضبطة ورصانة تناسب جلال قصص الأنبياء وسيرة الرسول.
...
export async function generateEpisodeMoralLessons({
  ...
  const prompt = `أنت عالم ومربٍّ إسلامي متخصص في فقه قصص الأنبياء وسير الرسل واستنباط العبر الإيمانية والتربوية.
اقرأ نص هذه الحلقة: «${title}»

نص الحلقة:
${plainText}

المطلوب بدقة:
استخرج من هذا الحدث ومواقف الأنبياء وسير الرسل الكرام أهم 3 إلى 5 دروس وعبر إيمانية وتربوية وعملية تفيد المسلم في حياته المعاصرة.
